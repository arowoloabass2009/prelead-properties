...
interface PropertyListing {
  id: string;
  title: string;
  type: 'residential' | 'commercial' | 'multipurpose' | 'multinational' | 'plaza' | 'mooreland' | 'industrial';
  price: number;
  location: string;
  state: string;
  bedrooms?: number;
...
const generateId = (): string =>
  'PLP-' + Math.random().toString(36).substring(2, 10).toUpperCase();
...
// ─────────────────── Toast ───────────────────────────────────
class Toast implements ToastAPI {
...
class ScrollAnimator {
  ...
  refresh(): void {
    document.querySelectorAll(
      '.reveal:not(.visible),.reveal-up:not(.visible),.reveal-left:not(.visible),.reveal-right:not(.visible),.reveal-scale:not(.visible)'
    ).forEach(el => this.observer.observe(el));
  }
}
...
const animateCounter = (el: HTMLElement, target: number, duration = 2200): void => {
  ...
  const fmt     = (n: number): string => (useSep ? n.toLocaleString() : String(n));
...
class WishlistManager {
  private static KEY = 'prelead_wishlist';
  ...
  static updateUI(): void {
    ...
    if (count) count.textContent = String(this.getCount());
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const pwInput = document.querySelector<HTMLInputElement>('#password, [data-pw-strength]');
  const pwSegs  = document.querySelectorAll<HTMLElement>('.pw-seg');
  const pwLabel = document.querySelector<HTMLElement>('.pw-label');
  if (pwInput) {
  ...
  document.querySelectorAll<HTMLElement>('[data-wishlist]').forEach(btn => {
    ...
    if (WishlistManager.has(id)) btn.classList.add('saved');
    btn.addEventListener('click', () => {
